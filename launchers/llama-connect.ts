...
export interface LlamaConnectConfig {
  sshTarget: string;
  remotePort: number;
  localPort: number;
  modelName: string;
...
const CONFIG_PATH = path.join(os.homedir(), '.llama-connect.json');

const DEFAULT_CONFIG: LlamaConnectConfig = {
  sshTarget: '',
  remotePort: 8080,
  localPort: 8080,
  modelName: ''
...
export function loadConfig(): LlamaConnectConfig {
  try {
...
export function saveConfig(cfg: LlamaConnectConfig): void {
  try {
...
async function main(): Promise<void> {
  console.clear();
  console.log('====================================================');
  console.log('         llama.cpp Server WebUI Connector           ');
  console.log('====================================================\n');
  ...
  const WEBUI_URL = `http://localhost:${config.localPort}`;
  ...
  if (isUp) {
    ...
    console.log(`> WebUI URL: ${WEBUI_URL}\n`);
    copyToClipboard(WEBUI_URL);
    console.log('[*] URL copied to clipboard!');
    console.log('[2/3] Opening browser to llama.cpp WebUI...');
  ...
  if (connected) {
    ...
    console.log('====================================================');
    console.log(`\n  WebUI URL:   ${WEBUI_URL}`);
    console.log(`  Model:       ${config.modelName || 'llama-server'}`);
    console.log('\n  [*] URL automatically copied to clipboard!');
    console.log('  [*] Opening web browser to llama.cpp WebUI...');
    console.log('  --------------------------------------------------');
    ...
    console.log(`You can still try opening ${WEBUI_URL} in your browser.`);
  }
